...
async function fetchNotes() {
    try {
        ...
        notesContainer.innerHTML = `<p class="no-data">Data Note Found</p>`;
    }
}
...
function renderNotes(notes) {
    ...
    if (notes.length === 0) {
        notesContainer.innerHTML = `<p class="no-data">Data Not Found. ${filterDateInput.value}</p>`;
        return;
    }
...
function triggerEdit(id, date, encodedText) {
    ...
    submitBtn.innerHTML = `<span>Update Note    </span>`;
...
function resetForm() {
    ...
    submitBtn.innerHTML = `<span>Save Note</span>`;
}
